...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.72\textwidth]{figures/fig_overview.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.68\textwidth]{figures/fig_architecture.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{figures/fig_createflow.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.70\textwidth]{figures/fig_verifypipe.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\textwidth]{figures/fig_protocol.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{figures/fig_lifecycle.png}
...
伪造份额 & 提交篡改的shareValue & 任意攻击者 & Pedersen承诺验证，伪造在第一层即被拒绝。\\
跨计划注入 & 将A计划的份额提交到B计划 & 持有他计划份额的监护人 & 承诺绑定到特定多项式，不同计划的系数不同，验证失败。\\
份额重用 & 从公开存储获取$Y_i$假装知$y_i$ & 可访问plans.json & Pedersen验证等式$zG=R+eY_i$，攻击者不知$y_i$无法构造合法$z$。\\
提前继承 & 时间锁到期前发起继承 & 继承人 & 后端检查时间锁剩余时间；合约检查block.timestamp。\\
重复提交 & 同一监护人多次提交 & 恶意监护人 & submittedGuardians数组防重（后端）+hasSubmitted标志（合约）。\\
服务器泄露 & 读取plans.json等文件 & 服务器权限攻击者 & StoredShare不含value；encryptedAssets需主密钥解密；主密钥被门限拆分。\\
链上篡改 & 直接调用合约函数 & 以太坊账户攻击者 & require条件、onlyOwner和onlyGuardian修饰符限制权限。\\
...
SHA256(Fiat-Shamir) & 128位碰撞安全 & 随机预言机模型 & 碰撞$2^{128}$次，原像$2^{256}$\\
